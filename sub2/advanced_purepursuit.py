...
class pure_pursuit :
    ...
    def calc_pure_pursuit(self,):
        ...
        '''
        self.lfd = self.lfd_gain*self.target_velocity
...
class velocityPlanning:
    ...
    def curvedBaseVelocity(self, gloabl_path, point_num):
        ...
        for i in range(point_num, len(gloabl_path.poses) - point_num):
            ...
            xtx_inv = np.linalg.pinv(np.dot(x_array.T, x_array))
            coord = np.dot(np.dot(xtx_inv, x_array.T), y_array)
            r = sqrt(coord[0]**2+coord[1]**2-coord[2])
