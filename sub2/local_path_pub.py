    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_odom == True and self.is_path == True:
                ...
                min_dis = float('inf')
                current_waypoint = -1
                for now_idx in range(len(self.global_path_msg.poses)):
                    now_point = PoseStamped()
                    now_point = self.global_path_msg.poses[now_idx]
                    dis = sqrt((x-now_point.pose.position.x)**2+(y-now_point.pose.position.y)**2)
                    if(min_dis>dis):
                        min_dis = dis
                        current_waypoint = now_idx
                ...
                if current_waypoint != -1 :
                    if current_waypoint + self.local_path_size < len(self.global_path_msg.poses):
                        for now_idx in range(current_waypoint, len(self.global_path_msg.poses)):
                            local_path_msg.poses.append(self.global_path_msg.poses[now_idx])    
                    else :
                        continue
                print(x,y)
