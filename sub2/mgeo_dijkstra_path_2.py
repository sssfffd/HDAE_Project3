...
class dijkstra_path_pub :
    ...
    def init_callback(self,msg):
        ...
        for node_idx in self.nodes.keys() :
                ...
                dis = sqrt((node_x-msg.pose.pose.position.x)**2+(node_y-msg.pose.pose.position.y)**2)
                if(min_dis>dis):
                    min_dis = dis
                    nodepoint = node_idx

        self.start_node = nodepoint
    ...
    def goal_callback(self,msg):
        ...
        for node_idx in self.nodes.keys() :
                node_x=self.nodes[node_idx].point[0]
                node_y=self.nodes[node_idx].point[1]
                dis = sqrt((node_x-msg.pose.position.x)**2+(node_y-msg.pose.position.y)**2)
                if(min_dis>dis):
                    min_dis = dis
                    nodepoint = node_idx
        
        self.end_node = nodepoint
