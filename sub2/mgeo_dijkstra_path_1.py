...
class dijkstra_path_pub :
    ...
    def calc_dijkstra_path_node(self, start_node, end_node):
        ...
        '''
        for i in range(len(path['point_path'])):
            read_pose = PoseStamped()
            read_pose.pose.position.x = float(path['point_path'][i][0])
            read_pose.pose.position.y = float(path['point_path'][i][1])
...
class Dijkstra:
    ...
    def get_weight_matrix(self):
        ...
        # 모든 from Node 에서 to Node 무한대로 Weight 설정
