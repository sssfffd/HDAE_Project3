...
class lc_path_pub :
    def __init__(self):
        ...
        self.global_path_pub=rospy.Publisher("/global_path",Path,queue_size=0)
        self.local_path_pub = rospy.Publisher('/lane_change_path',Path,queue_size=0)
    ...
    def check_object(self,ref_path,global_vaild_object,local_vaild_object):
        ...
        if len(global_vaild_object) >0  :
            ...
            for i in range(len(global_vaild_object)):
                for path in ref_path.poses :   
                    if global_vaild_object[i][0]==1 or global_vaild_object[i][0]==2 :  
                        dis = sqrt((global_vaild_object[i][1]-path.pose.position.x)**2+(global_vaild_object[i][2]-path.pose.position.y)**2)
                        if dis<2.5:
    ...
    def getLaneChangePath(self,ego_path,lc_path,start_point,end_point,start_next_point, end_waypoint_idx): ## 
        ...
        point_to_point_distance = 0.5
        start_path_distance = sqrt((end_point.pose.position.x-start_next_point.pose.position.x)**2+(end_point.pose.position.y-start_next_point.pose.position.y)**2)
        start_path_repeat = int(start_path_distance/point_to_point_distance)

        theta = atan2(end_point.pose.position.y - start_next_point.pose.position.y, end_point.pose.position.x - start_next_point.pose.position.x)
        ...
        for k in range(0,start_path_repeat+1):
            ...
            roation_matric_calc = np.matmul(ratation_matric_1,ratation_matric_2)
            read_pose=PoseStamped()
            read_pose.pose.position.x = start_next_point.pose.position.x+roation_matric_calc[0][0]
            read_pose.pose.position.y = start_next_point.pose.position.y+roation_matric_calc[1][0]
            read_pose.pose.position.z = 0
            read_pose.pose.orientation.x = 0
            read_pose.pose.orientation.y = 0
            read_pose.pose.orientation.z = 0
