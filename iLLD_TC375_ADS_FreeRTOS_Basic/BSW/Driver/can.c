...
/*-----------------------------------------------Weak Callbacks------------------------------------------------------*/
/*********************************************************************************************************************/
__attribute__((weak)) void Can_RxUserCallback(unsigned int id, const char *data, int len)
{
    (void)id;
    (void)data;
    (void)len;
...
static volatile uint32 g_can_rx_isr_cnt = 0;
...
void Can_RxIsrHandler (void)
{
    ...
    /* Lightweight debug to confirm ISR firing (prints sparsely) */
    // if ((g_can_rx_isr_cnt & 0x3F) == 1) /* every 64th interrupt */
    // {
    //     my_printf("CAN RX ISR: id=0x%03X len=%d cnt=%u\n",
    //               (unsigned)rxID, (int)rxLen, (unsigned)g_can_rx_isr_cnt);
    // }
    Can_RxUserCallback(rxID, rxData, rxLen);
}
